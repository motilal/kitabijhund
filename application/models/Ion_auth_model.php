...
class Ion_auth_model extends CI_Model {
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $query = $this->db->select($this->identity_column . ', email, id, password, active, two_step_authentication, last_login')
                ->where($this->identity_column, $identity)
                ->limit(1)
                ->order_by('id', 'desc')
                ->get($this->tables['users']);
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                if ($user->two_step_authentication == 1) {
                    $this->trigger_events('post_login_unsuccessful');
                    $this->TwoStepAuthEmail();
                    return FALSE;
                }
                if ($user->active == 0) {
                    $this->trigger_events('post_login_unsuccessful');
    ...
    private function TwoStepAuthEmail($code) {
